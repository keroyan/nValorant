...
void* ImGui_CreateTextureRGBA(int width, int height, const unsigned char* data)
{
    ...
    if (ImGui_ImplDX9_GetBackendData()->pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &tempTexture, nullptr) != D3D_OK)
        return nullptr;
    ...
    if (ImGui_ImplDX9_GetBackendData()->pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_AUTOGENMIPMAP, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &texture, nullptr) != D3D_OK) {
        tempTexture->Release();
        return nullptr;
    }

    ImGui_ImplDX9_GetBackendData()->pd3dDevice->UpdateTexture(tempTexture, texture);
    tempTexture->Release();
